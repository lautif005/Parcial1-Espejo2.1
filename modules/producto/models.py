...
class ProductoCategoria(SQLModel, table=True):
    ...
    producto_id: int = Field(default=None, sa_column=Column(BigInteger(), ForeignKey("producto.id"), primary_key=True))
    categoria_id: int = Field(default=None, sa_column=Column(BigInteger(), ForeignKey("categoria.id"), primary_key=True))
...
class ProductoIngrediente(SQLModel, table=True):
    ...
    producto_id: int = Field(default=None, sa_column=Column(BigInteger(), ForeignKey("producto.id"), primary_key=True))
    ingrediente_id: int = Field(default=None, sa_column=Column(BigInteger(), ForeignKey("ingrediente.id"), primary_key=True))
...
class Producto(SQLModel, table=True):
    ...
    precio_base: float = Field(default=0.0, sa_column=Column(DECIMAL(10, 2), nullable=False))
